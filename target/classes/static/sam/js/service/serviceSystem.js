...
require(["load", "request", "jquery", 'ajaxUtils','crm', 'common-enum', 'easyui', 'backspace'], function (load, request, $,AjaxUtils, CRM, ENUM) {
    ...
    function initSearchGrid() {
        $search = $([
            ...
            "<div class='row cl'>",
            "<label class='form-label col-2 '>系统工号：</label>",
            "<div class='formControls col-2'>" +
            "<input type='text' class='easyui-textbox' name='serviceId' style='width:100%;height:30px' >" +
            "</div>",

            "<label class='form-label col-2'>系统名称：</label>",
            "<div class='formControls col-2'>" +
            "<input type='text' class='easyui-textbox' name='serviceName' style='width:100%;height:30px' >" +
            "</div>",
    ...
    function initAccountGrid() {
        ...
        $([
            ...
            "<div class='fl text-bold'>业务系统列表</div>",
            "<div class='fr'>",
        ...
        $page.find("#account").datagrid({
            columns: [[
                ...
                {field: 'serviceId', title: '系统ID', width: '17%'},
                {field: 'serviceName', title: '系统名称', width: '22%'},
                {field: 'clientId', title: '客户端ID', width: '22%'},
                {
                ...
                {field: 'serviceDesc', title: '备注', width: '20%'}
            ]],
            fitColumns: true,
            rownumbers: true,
            striped: true,
            ...
            loader: function (param, success) {
                ...
                AjaxUtils.commonAjax('post','/sam/tsamservicesystem/qryServiceSystem',params,function (data) {
    ...
    function initPopWindow() {
        ...
        $([
            ...
            "</div>",
            "<div class='row cl'>",
            "<label class='form-label col-2'>系统ID</label>",
            "<div class='formControls col-2'><input  type='text' id='serviceId' name='serviceId' class='easyui-textbox'  style='width:90%;height:30px' /><span style='color:red;padding-left:2px'>*</span></div>" +
            "<label class='form-label col-2'>系统名称</label>" +
            "<div class='formControls col-2'><input  type='text' id='serviceName' name='serviceName' class='easyui-textbox' style='width:90%;height:30px' /><span style='color:red;padding-left:2px'>*</span></div>" +
            "</div>",
            "<div class='row cl'>",
            "<label class='form-label col-2'>客户端ID</label>" +
            "<div class='formControls col-2'><input  type='text' id='clientId' name='clientId' class='easyui-textbox' style='width:90%;height:30px' /><span style='color:red;padding-left:2px'>*</span></div>" +
            "<label class='form-label col-2'>状态</label>" +
            ...
            "<div class='formControls col-8'><input  type='text' id ='serviceDesc' name='serviceDesc' class='easyui-textbox' style='width:100%;height:55px' /></div>" +
            "</div>",
            "</form>",
            "</div>"
    ...
    function initUpdatePopWindow() {
        ...
        $([
            ...
            "<div class='row cl'>",
            "<label class='form-label col-2'>系统ID</label>",
            "<div class='formControls col-2'><input  type='text' name='serviceId' class='easyui-textbox' disabled='true' style='width:90%;height:30px' /><span style='color:red;padding-left:2px'>*</span></div>" +
            "<label class='form-label col-2'>系统名称</label>" +
            "<div class='formControls col-2'><input  type='text' id='serviceName' name='serviceName' class='easyui-textbox' style='width:90%;height:30px' /><span style='color:red;padding-left:2px'>*</span></div>" +
            "</div>",
            "<div class='row cl'>",
    ...
    function initWindowEvent() {
        $popWindow.on("click", "#global", function () {
            ...
            var serviceId = $popWindow.find("input[id='serviceId']").val().trim();
            var serviceName = $popWindow.find("input[name='serviceName']").val().trim();
            var clientId = $popWindow.find("input[name='clientId']").val().trim();
            var state=$popWindow.find("select[id='state']").val().trim();
            if (!serviceId) {
                $.messager.alert('提示', '请填写系统ID！');
                $(this).linkbutton({disabled: false});
            }else if(!serviceName){
                $.messager.alert('提示', '请填写系统名称！');
                $(this).linkbutton({disabled: false});
            }else if(!clientId){
                ...
                AjaxUtils.commonAjax('post','/sam/tsamservicesystem/createServiceSystem',sd,function (data) {
    ...
    function initUpdateWindowEvent() {
        $popWindow.on("click", "#globalUpd", function () {
            ...
            var serviceId = $popWindow.find("input[name='serviceId']").val().trim();
            var serviceName = $popWindow.find("input[name='serviceName']").val().trim();
            var clientId = $popWindow.find("input[name='clientId']").val().trim();
            if (!serviceId) {
                $.messager.alert('提示', '请填写系统ID！');
                ...
                var sd = getParams($popWindow);
                AjaxUtils.commonAjax('post','/sam/tsamservicesystem/updServiceSystem',sd,function (data) {
                    $("#global").linkbutton({disabled: false});
